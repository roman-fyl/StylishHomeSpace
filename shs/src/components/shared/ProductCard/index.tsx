...
export interface ProductCardProps {
  ...
  brandText: string;
  price: number;
  imageSlider: Record<string, any>[];
  imageSliderLink: string;
}

export const ProductCard: FC<{ item: ProductCardProps }> = ({ item }) => {
  ...
  useEffect(() => {
    ...
    const itemExists = storedWishlist.some(
  ...
  const GenerateOldPrice = (price: any, percentage: any) =>
    price * (1 + percentage / 100);
  const calculateDiscountedAmount = (price: any, percentage: any) =>
        ...
        <div className="item_description">
          ...
          <ul className="item_tags">
            {item.tags.map((tag, index) => (
              <li key={index}>
                <img src={tag.iconLink} alt={`${tag.value} ${item.sku}`}></img>
              </li>
            ))}
          </ul>
          ...
          <span className="item_pricing">
            <span className="item_old-price">
              <del>
                $
                {GenerateOldPrice(parseFloat(`${item.price}`), 12.319).toFixed(
                  2
                )}
              </del>
            </span>
